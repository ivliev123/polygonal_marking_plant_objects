...
# === НАСТРОЙКИ ПАПОК ===
INPUT_IMAGE_FOLDER = "input_images/"  # Папка для входных изображений
OUTPUT_PHOTO_FOLDER = "output_photos_RU/"  # Папка для выходных фотографий
    ...
    for i in range(15):  # Всегда 15 ячеек
        if i < n_show:  # Есть данные для этого индекса
            ...
            if result['contour'] is not False and result['contour'] is not None:
                ...
                title = (f"Цветовой диапазон=[{result['h_lower']},{result['h_upper']}]\n"
                        # f"Score={result['score']:.2f}\n")
                        # f"Интегральная оценка \nкачества сегментации={result['score']:.2f}\n")
                        )
...
def visualize_score_progression(history, best_index, output_dir, image_name, class_name, obj_id, img_name_article):
    ...
    ax.set_xlabel('Индекс цветового диапазона')
    ax.set_ylabel('Интегральная оценка качества сегментации')
...
def visualize_area_progression(history, best_index, output_dir, image_name, class_name, obj_id, img_name_article):
    ...
    ax.set_xticks(x_positions)  # Каждый индекс
    ax.set_xlabel('Индекс цветового диапазона')
    ax.set_ylabel('Площадь контура, пикс.$^2$')
    ax.grid(True, alpha=0.3)
    ...
    for i in range(15):  # Всегда 15 ячеек
        if i < n_show:  # Есть данные для этого индекса
            ...
            if result['contour'] is not False and result['contour'] is not None:
                ...
                # Формируем заголовок с метрикам
                title = (f"Цветовой диапазон=[{result['h_lower']},{result['h_upper']}]\n"
                        f"Интегральная оценка \nкачества сегментации={result['score']:.2f}\n")
            else:
                # Нет контура - показываем исходное изображение без контура
...
def process_single_image_by_index(image_index=0, path_images=INPUT_IMAGE_FOLDER, output_root=OUTPUT_PHOTO_FOLDER, PATH_ = True):
    """
...
def main():
    # === НАСТРОЙКИ ===
    # Используем глобальные переменные папок
    path_images = INPUT_IMAGE_FOLDER
    ...
    RANGE_START = 0  # для MODE=RANGE
    RANGE_END = 1   # включительно

    if MODE == "SINGLE":
        output_root = OUTPUT_PHOTO_FOLDER
        results, viz_paths = process_single_image_by_index(IMAGE_INDEX, path_images, output_root, PATH_ = True)
