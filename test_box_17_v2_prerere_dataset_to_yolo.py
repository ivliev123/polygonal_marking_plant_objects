...
def visualize_all_segmentation(image, masks_data, save_path):
    ...
    for i, mask_data in enumerate(masks_data):
        ...
        # Рисуем полупрозрачную маску
        colored_mask = np.zeros_like(vis_image)
        colored_mask[mask_data['mask'] > 0] = color
    ...
    # Добавляем легенду
    legend_y = 30
    for i in range(min(len(masks_data), len(colors))):
        color = colors[i]
        # cv2.putText(vis_image, f'Contour {i}', (10, legend_y), 
...
def main():
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        # === 7. Визуализация ВСЕХ сегментаций на одном изображении ===
        if all_masks_data:
        ...
        print(f"   Найдено контуров: {len(all_masks_data)}")
